...
impl<A> Service<A> {
    ...
    ///
    /// A TCP endpoint configured with port 0 binds a new socket on each
    /// process start and may report a different port after restart.
...
impl<A: ServerApp + Send + Sync + 'static> ServiceTrait for Service<A> {
    ...
    fn listen_addresses(&self) -> Option<Vec<String>> {
        #[cfg(unix)]
        {
            Some(self.listeners.transferable_addresses())
        }
        #[cfg(not(unix))]
        {
